...
$(document).ready(function () {
	...
	$('[data-plugin="select_hrm_small"]').select2($(this).attr("data-options"));
	$('[data-plugin="select_hrm_small"]').select2({ width: "resolve" });
	...
	/* Add project file */ /*Form Submit*/ $("#add_attachment").submit(function (
		e
	) {
		var fd = new FormData(this);
		var obj = $(this),
	...
	$(".edit-modal-variation-data").on("show.bs.modal", function (event) {
		...
		$.ajax({
			...
			type: "GET",
			data:
				"jd=1&is_ajax=1&mode=modal&data=project_variation&variation_id=" +
				variation_id +
				"&mname=" +
				mname,
			success: function (response) {
	...
	$(".edit-modal-timelog-data").on("show.bs.modal", function (event) {
		...
		$.ajax({
			...
			type: "GET",
			data:
				"jd=1&is_ajax=1&mode=modal&data=project_timelog&timelogs_id=" +
				timelogs_id,
			success: function (response) {
				if (response) {
	...
	var xin_variation_table = $("#xin_variation_table").dataTable({
		...
		ajax: {
			url:
				site_url +
				"timesheet/project_variation_list/" +
				$("#tproject_id").val(),
			type: "GET",
		},
		fnDrawCallback: function (settings) {
	...
	$("#redelete_timelog").submit(function (e) {
		...
		$.ajax({
			...
			url: e.target.action,
			data:
				obj.serialize() + "&is_ajax=8&data=timelog&type=delete&form=" + action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != "") {
					toastr.error(JSON.error);
...
$(document).on("click", ".delete-variation", function () {
	$("input[name=_token_del_file]").val($(this).data("record-id"));
	$("#delete_record_v").attr(
		"action",
		site_url + "timesheet/delete_variation/" + $(this).data("record-id")
	);
});
$(document).on("click", ".delete-timelog", function () {
	$("input[name=_token_timelog]").val($(this).data("record-id"));
	$("#redelete_timelog").attr(
		"action",
		site_url + "project/delete_timelog/" + $(this).data("record-id")
	);
});
